...
class BinanceFuturesClient:
    ...
    def make_request(self, method, endpoint, data):
        ...
        if response.status_code == 200:
            ...
            logger.error("Erro while making requests to %s (error code %s)",method, endpoint, response.json(), response.status_code)
            return None
    ...
    def place_order(self, symbol, side, qunatity, order_type, price=None, tif=None):
        data = dict()
        data['symbol'] = symbol
        data['side'] = side
    ...
    def on_open(self, ws):
        ...
        self.subscribe_channel("BITCUSDT")
